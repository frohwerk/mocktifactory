...
func StorageHandler(rw http.ResponseWriter, r *http.Request) {
	encoder := json.NewEncoder(rw)
	encoder.SetIndent("", "  ")

	path := strings.TrimPrefix(r.URL.Path, "/api/storage/libs-release-local")
	...
	if fsinfo, err := os.Stat(location); os.IsNotExist(err) {
		...
		if files, err := ioutil.ReadDir(location); err != nil {
			...
			rw.WriteHeader(http.StatusOK)
				...
				Uri:          fmt.Sprintf("http://%s%s", r.Host, r.URL),
			}
